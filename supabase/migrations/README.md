...
### `20260128_backfill_existing_customer_points.sql` - 기존 회원 포인트 소급 적용 ⭐

**내용:**
- 기존 회원들의 `total_point_general`, `total_point_betting` 잔액을 `points` 테이블에 초기 레코드로 생성
- `approved` 상태로 생성 (이미 사용 중인 포인트이므로)
- note에 "시스템 소급 적용" 표시로 구분 가능
- 중복 방지: 이미 소급 적용된 회원은 스킵
...
**적용 효과:**
- 기존 회원들의 포인트 이력이 `points` 테이블에 기록됨
- 포인트 관리 페이지의 "전체 장부"에서 초기 포인트 확인 가능
- 승인/거절 프로세스와 동일한 데이터 구조로 통일

**⚠️ 주의사항:**
- 이 마이그레이션은 **한 번만 실행**해야 합니다
- 실행 전 현재 회원들의 포인트 잔액을 백업하는 것을 권장합니다
